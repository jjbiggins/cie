...
=======
  const bool found = binary_search(names.begin(), names.end(), who);
>>>>>>> .r108763
...
int main()
{
  ...
  for( const double item : darr )  //Range based for loop from Ch. 7.5
  {
     cout << item << endl;
  }
  cout << "----------------------" << endl;
  ...
  for(size_t i = 0; i <  (iarr.size() / 2); i++){
    cout << iarr[i] << endl;
  }
  ...
  for(size_t i = (usarr.size() / 2); i < usarr.size(); i++){
    cout << usarr[i] << endl;
  }
  ...
  for(unsigned short &itemRef : usarr){
    itemRef = 79;
  }
  for( int item : usarr )
  {
