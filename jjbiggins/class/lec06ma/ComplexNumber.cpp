...
void ComplexNumber::setRealPart(double real) { m_real = real; }

void ComplexNumber::setImagPart(double imag) { m_imag = imag; }
...
double ComplexNumber::getRealPart() const { return m_real; }

double ComplexNumber::getImagPart() const { return m_imag; }

double ComplexNumber::getMagnitude() const { return sqrt(m_real * m_real + m_imag * m_imag); }

double ComplexNumber::getPhaseAngleInRadians() const { return atan2(m_imag, m_real); }

double ComplexNumber::getPhaseAngleInDegrees() const { return atan2(m_imag, m_real) * 45 / atan(1); }
...
void ComplexNumber::printInCartesianForm() const { std::cout << m_real << " + j" << m_imag << std::endl; }

void ComplexNumber::printInPolarFormRadians() const
{
  std::cout << "magnitude = " << getMagnitude() << ", angle = " << getPhaseAngleInRadians() << " radians" << std::endl;
}

void ComplexNumber::printInPolarFormDegrees() const
{
  std::cout << "magnitude = " << getMagnitude() << ", angle = " << getPhaseAngleInDegrees() << " degrees" << std::endl;
}
